...
{
    public class AdvantageValidator : AbstractValidator<Advantage>
    {
        public AdvantageValidator()
        {
            RuleFor(a => a.TagLine).NotEmpty().WithMessage("Üst bilgiyi boş geçemezsiniz!");
            RuleFor(a => a.Icon).NotEmpty().WithMessage("İcon bilgisini boş geçemezsiniz!");
